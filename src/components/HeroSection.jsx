...
export const HeroSection = () => {
  ...
  // Optimized typing animation to reduce DOM thrashing
  useEffect(() => {
  ...
  // Enhanced scroll prevention effect - optimized to reduce reflows
  useEffect(() => {
    ...
    const updateScrollLock = (locked) => {
      requestAnimationFrame(() => {
        ...
        if (locked) {
          ...
          // Use requestAnimationFrame for scroll reset to avoid forced reflow
          requestAnimationFrame(() => {
            window.scrollTo(0, 0);
          });
        } else {
    ...
    return () => {
      ...
      // Restore scrolling when component unmounts - batched
      requestAnimationFrame(() => {
  ...
  return (
    <>
      {/* Move styles to the document head or use CSS modules */}
